...
import type { DailyPlan, PlanItem, DayStatus } from '@/lib/planner';
...
const CATEGORY_TONE: Record<AiCategory, { dot: string; chipBg: string; chipText: string; chipBorder: string }> = {
...
const STATUS_TONE: Record<DayStatus, { ring: string; pillBg: string; pillText: string; label: string }> = {
...
// Build a Map<YYYY-MM-DD, DailyPlan> from runway so month view can look
// up planned days by date without depending on dayIndex order.
function indexRunway(runway: DailyPlan[]): Map<string, DailyPlan> {
  const m = new Map<string, DailyPlan>();
  for (const day of runway) {
    // DailyPlan.date may be a full ISO or just date — normalize to YYYY-MM-DD.
